...
class CallValidatorTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_invalid_signature(self):
        ...
        self.assertEqual(
            validation.response[0].decode(), "Store signature is not valid"
        )
    ...
    def test_valid_paid_call(self):
        ...
        self.assertEqual(
            ...
            "Call is valid redirected to {}".format(
                functions.shopify_topics_handler[self.request.headers["X-Shopify-Topic"]]
            )
        )

    def test_valid_create_product_call(self):
        self.request.headers["X-Shopify-Topic"] = "products/create"
        validation = functions.call_validator(self.request)
        self.assertEqual(
            validation.response[0].decode(),
            "Call is valid redirected to {}".format(
